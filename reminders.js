(() => {
  ...
  async function seedConfiguredPresets() {
    const trip = window.LuviaTripContext?.getActiveTrip?.() || (() => { try { return JSON.parse(localStorage.getItem('parisIdentityV1') || 'null'); } catch { return null; } })();
    const ids = trip?.moduleSettings?.reminderPresets || [];
    if (!ids.length) return;
    const marker = `luvia-reminder-presets-v1:${trip?.tripId || 'local'}`;
    if (localStorage.getItem(marker)) return;
    const labels = { photo:'Familien- oder Gruppenfoto', localFood:'Etwas Typisches probieren', sunset:'Einen Sonnenuntergang genießen', souvenir:'Ein besonderes Souvenir finden', quietMoment:'Einen stillen Lieblingsmoment festhalten' };
    for (const id of ids) {
      const title = labels[id];
      if (!title || customRows.some(row => row.title === title)) continue;
      try { const created = await window.ParisSync.reminders.createCustom(title); customRows.push(created); }
      catch (error) { console.warn('Konfigurierter Erinnerungsvorschlag konnte nicht angelegt werden:', error); }
    }
    localStorage.setItem(marker, '1');
  }
