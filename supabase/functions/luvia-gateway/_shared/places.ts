...
if(action==='places.health'){result={status:'ok',service:'multi-provider-places-gateway',version:'4.28.7',configured:Boolean(getKey()||getFoursquareKey()),providers:{google:{configured:Boolean(getKey())},foursquare:{configured:Boolean(getFoursquareKey()),apiVersion:FOURSQUARE_API_VERSION}},metrics:{...metrics},cache:{entries:cache.size}};return{data:result,cache:{hit:false,key:null,ttlMs:0}};}
...
else if(action==='places.details'){const rawId=String(payload?.placeId||'').replace(/^places\//,'');if(rawId.startsWith('fsq:')){const fsqId=rawId.slice(4);const raw=await foursquare(`/places/${encodeURIComponent(fsqId)}`,{fields:'fsq_place_id,name,location,geocodes,categories,chains,distance,link,related_places,timezone,hours,rating,stats,price,photos,description,tastes,tel,website'});result={place:normalizedFoursquarePlace(raw)}}else{const id=encodeURIComponent(rawId);const raw=await google(`/places/${id}?languageCode=${encodeURIComponent(languageCode)}&regionCode=${encodeURIComponent(regionCode)}`,{method:'GET'},DETAIL_FIELDS);result={place:normalizedPlace(raw)}};}
