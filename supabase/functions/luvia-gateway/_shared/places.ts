...
const FOURSQUARE_API_VERSION=Deno.env.get('FOURSQUARE_API_VERSION')||'2025-06-17';
const FOURSQUARE_SEARCH_FIELDS='fsq_place_id,name,location,categories,chains,distance,link,related_places,hours,rating,stats,price,photos,description,tastes,tel,website';
const TIMEZONE_BASE='https://maps.googleapis.com/maps/api/timezone/json';
const cache=new Map<string,{expires:number,value:unknown}>();
const metrics={requests:0,successes:0,failures:0,cacheHits:0,resolutions:0,timezoneRequests:0,timezoneFailures:0,lastRequestAt:null as string|null,lastSuccessAt:null as string|null,lastError:null as unknown,providers:{google:{requests:0,successes:0,failures:0},foursquare:{requests:0,successes:0,failures:0}}};
...
async function foursquare(path:string,params:Record<string,unknown>={}){
  ...
  if(!response.ok){metrics.failures++;metrics.providers.foursquare.failures++;metrics.lastError={provider:'foursquare',status:response.status,body};throw Object.assign(new Error(body?.message||body?.error?.message||`Foursquare Anfrage fehlgeschlagen (${response.status}).`),{code:'FOURSQUARE_PROVIDER_ERROR',status:response.status,provider:body});}
  metrics.successes++;metrics.providers.foursquare.successes++;metrics.lastSuccessAt=new Date().toISOString();return body;
}
function fsqPhotoUri(photo:any,width=900){if(!photo)return null;if(photo.url)return photo.url;if(photo.prefix&&photo.suffix)return `${photo.prefix}${width}x${width}${photo.suffix}`;return null;}
function normalizedFoursquarePlace(p:any){
  const location=p.location||{};const geocodes=p.geocodes||{};const main=geocodes.main||geocodes.roof||{};
  const latitude=Number(main.latitude??location.latitude??p.latitude);const longitude=Number(main.longitude??location.longitude??p.longitude);
  const categories=(p.categories||[]).map((c:any)=>c?.name||c?.short_name).filter(Boolean);
  const categoryIds=(p.categories||[]).map((c:any)=>String(c?.id||'')).filter(Boolean);
  const photos=(p.photos||[]).map((x:any)=>({name:`foursquare/${p.fsq_place_id||p.id}/photos`,uri:fsqPhotoUri(x),widthPx:x.width||null,heightPx:x.height||null,authorAttributions:[]}));
  const id=String(p.fsq_place_id||p.id||'');
  return {id:`fsq:${id}`,providerPlaceId:`fsq:${id}`,resourceName:`foursquare/${id}`,provider:'foursquare',source:'foursquare_places',name:p.name||'',displayName:p.name||'',formattedAddress:location.formatted_address||[location.address,location.locality,location.region].filter(Boolean).join(', '),shortAddress:location.address||location.locality||'',country:location.country||'',countryCode:String(location.country||'').toUpperCase(),location:Number.isFinite(latitude)&&Number.isFinite(longitude)?{latitude,longitude}:null,primaryType:categoryIds[0]||'',primaryTypeLabel:categories[0]||'',types:[...categoryIds,...categories],rating:p.rating??null,userRatingCount:p.stats?.total_ratings??p.popularity??0,priceLevel:p.price??null,businessStatus:p.closed_bucket==='VeryLikelyClosed'?'CLOSED_PERMANENTLY':'OPERATIONAL',openNow:p.hours?.open_now??null,openingHours:p.hours||null,website:p.website||p.social_media?.facebook_id||null,mapsUri:null,phone:p.tel||null,photos,editorialSummary:p.description||p.tastes?.slice?.(0,4)?.join(', ')||null,reviews:[],features:{servesVegetarianFood:null},providerRefs:{foursquare:id},evidence:[{provider:'foursquare',kind:'place-search',categories}],raw:p};
}
...
async function foursquareSearch(query:string,destination:any,options:any){const anchor=searchAnchor(destination,options);const near=destination?.canonicalCity?.name||destination?.name||'';const params:any={query,limit:Math.min(20,Number(options?.maxResultCount||10)),sort:'RELEVANCE',fields:FOURSQUARE_SEARCH_FIELDS};if(anchor)params.ll=`${anchor.latitude},${anchor.longitude}`;else if(near)params.near=near;if(Number(options?.maxDistanceMeters)>0)params.radius=Math.min(100000,Number(options.maxDistanceMeters));if(options?.openNow)params.open_now=true;const raw=await foursquare('/places/search',params);const rows=raw.results||raw.places||[];return rows.map(normalizedFoursquarePlace);}
...
if(action==='places.health'){result={status:'ok',service:'multi-provider-places-gateway',version:'4.28.7.2',configured:Boolean(getKey()||getFoursquareKey()),providers:{google:{configured:Boolean(getKey())},foursquare:{configured:Boolean(getFoursquareKey()),apiVersion:FOURSQUARE_API_VERSION}},metrics:{...metrics},cache:{entries:cache.size}};return{data:result,cache:{hit:false,key:null,ttlMs:0}};}
...
export function placesDiagnostics(){return{configured:Boolean(getKey()||getFoursquareKey()),providers:{google:Boolean(getKey()),foursquare:Boolean(getFoursquareKey())},metrics:{...metrics},cache:{entries:cache.size}};}
