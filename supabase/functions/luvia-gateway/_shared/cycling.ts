...
export async function cyclingAction(action:string,payload:any){
 if(action==='cycling.health')return{data:{status:'ok',service:'cycling-routes',version:'4.11.0',configured:true,providers:{discovery:'openstreetmap-overpass',approachRouting:'google-routes-bicycle'},metrics:{...metrics},cache:{entries:cache.size}}};
