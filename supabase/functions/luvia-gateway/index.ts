...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    switch(action){
      case 'system.health':
        data={status:'ok',service:'luvia-gateway',version:'3.0.2.10',time:new Date().toISOString(),authenticated:Boolean(userId),places:placesDiagnostics(),restaurants:restaurantDiagnostics()};
        break;
      default:
