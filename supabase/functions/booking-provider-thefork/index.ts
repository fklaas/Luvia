...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const {data:cap,error:capError}=await admin.from('booking_provider_capabilities').select('*').eq('provider_id','thefork').maybeSingle();
    if(capError)throw capError;
    if(!cap)return json({error:'THEFORK_CAPABILITY_MISSING'},500);
