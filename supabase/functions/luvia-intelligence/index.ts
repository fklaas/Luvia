...
Deno.serve(async(req:Request)=>{
  ...
  if(PUBLIC.has(action))return jsonResponse(200,{ok:true,data:{service:'luvia-intelligence',status:'ok',version:'4.34.0',build:'13.34.0',core:'4.34.0',authenticated:Boolean(userId),...modelDiagnostics(),capabilities:listCapabilities(),privacy:{store:false,promptsLogged:false,minimumNecessaryContext:true}},meta:{requestId:id}},cors);
