...
Deno.serve(async(req:Request)=>{
  ...
  const clientKey=req.headers.get('x-forwarded-for')?.split(',')[0]?.trim()||req.headers.get('cf-connecting-ip')||'unknown';const rate=enforceRateLimit(`${clientKey}:${action}`,PUBLIC.has(action)?60:20,60_000);if(!rate.allowed)return errorResponse(429,'RATE_LIMITED','Zu viele KI-Anfragen.',id,{...cors,'Retry-After':String(rate.retryAfter)});
  ...
  if(PUBLIC.has(action))return jsonResponse(200,{ok:true,data:{service:'luvia-intelligence',status:'ok',version:'4.22.0',build:'13.22.0',core:'4.22.0',authenticated:Boolean(userId),...modelDiagnostics(),capabilities:listCapabilities(),privacy:{store:false,promptsLogged:false,minimumNecessaryContext:true}},meta:{requestId:id}},cors);
