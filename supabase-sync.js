(() => {
  ...
  async function ensureTrip() {
    if (!window.LuviaEntry?.ensure) throw new Error('Luvia-Reiseeinstieg konnte nicht geladen werden.');
    identity = await window.LuviaEntry.ensure(client);
  ...
  async function stopRealtime() {
    ...
    if (current) {
      try { await client.removeChannel(current); } catch (error) { console.warn('Realtime-Kanal konnte nicht entfernt werden:', error?.message || error); }
    }
  }
  ...
  async function subscribe() {
    ...
    if (realtimeChannel && realtimeChannelKey === key) return realtimeChannel;
    await stopRealtime();
    const channel = client.channel(`paris-trip-${tripId}-${userId}`)
  ...
  function injectCloudUi() {
    ...
    modal.innerHTML = `
      <div class="paris-cloud-card" role="dialog" aria-modal="true" aria-labelledby="parisCloudTitle">
        ...
        <h2 id="parisCloudTitle">Unsere gemeinsame Reise</h2>
        <p id="parisCloudText">Dieses Gerät verbindet sich mit eurer gemeinsamen Reise, sobald eine Anmeldung aktiv ist.</p>
    ...
    toast.textContent = '☁️ Dieses Gerät ist mit eurer Luvia-Reise verbunden.';
    ...
    document.getElementById('parisCloudInvite').addEventListener('click', () => { close(); window.LuviaEntry?.showInvite(); });
  }
  ...
  function setCloudUi(status, message) {
    ...
    if (status === 'ready') {
      ...
      if (badge) { badge.textContent = '🔐 Abgemeldet'; badge.style.background = '#fff5e8'; badge.style.color = '#8b632b'; }
      if (text) text.textContent = 'Du bist auf diesem Gerät abgemeldet. Öffne Profil → Konto, um dich wieder anzumelden.';
      const ident = document.getElementById('parisCloudIdentity');
      if (ident) ident.textContent = 'Keine aktive Anmeldung';
    } else {
  ...
  async function start(options = {}) {
    ...
    try {
      ...
      if (!hasSession) {
        await stopRealtime();
        userId = null; tripId = null; identity = null;
        document.documentElement.dataset.cloudSync = 'signed-out';
        setCloudUi('signed-out');
        return;
      }
      await ensureTrip();
  ...
  window.ParisCloud.connect = (options = {}) => start({ force: true, ...options });
  window.ParisCloud.disconnectRealtime = stopRealtime;
  window.addEventListener('luvia:logout', () => stopRealtime());
  window.addEventListener('beforeunload', () => { if (realtimeChannel) client.removeChannel(realtimeChannel); });
  start();
})();
