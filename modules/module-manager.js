(function(){
  ...
  const current=()=>{
    ...
    return {
      ...stored,
      ...active,
      selectedModules:Array.isArray(active.selectedModules)&&active.selectedModules.length?active.selectedModules:(Array.isArray(stored.selectedModules)?stored.selectedModules:active.selectedModules),
  ...
  async function pushCloud(tripId,patch){const client=window.ParisCloud?.client;if(!client||!tripId)return;try{await window.ParisCloud.ready;const modules=Array.isArray(patch.selectedModules)?patch.selectedModules:[];const r=await client.rpc('luvia_set_trip_modules',{p_trip_id:tripId,p_modules:modules,p_settings:{...(patch.moduleSettings||{}),moduleContent:patch.moduleContent||undefined}});if(r.error)throw r.error}catch(error){console.info('Modulkonfiguration bleibt lokal:',error?.message||error)}}
  ...
  function openEditor(trip=current(),onSave){if(!trip?.tripId)return;if(isOfficialParis(trip)){alert('Die offizielle Paris-Reise bleibt als Komplettvorlage geschützt. Inhalte können künftig über eine Kopie individualisiert werden.');return}const modal=document.createElement('div');modal.className='lm-modal';modal.innerHTML=`<div class="lm-modal-card"><div class="lm-modal-head"><div><span class="lm-eyebrow">Reise konfigurieren</span><h2>Module dieser Reise</h2><p>${trip.symbol||'❤️'} ${trip.tripName||'Unsere Reise'} · Neue Modultechnik V2</p></div><button class="lm-close">×</button></div>${pickerHtml(trip.selectedModules,trip.moduleSettings,{trip,accent:trip.accent})}<div class="lm-actions"><button class="pc-btn" data-cancel>Abbrechen</button><button class="pc-btn primary" data-save>Änderungen speichern</button></div></div>`;document.body.appendChild(modal);bindPicker(modal);const close=()=>modal.remove();modal.querySelector('.lm-close').onclick=close;modal.querySelector('[data-cancel]').onclick=close;modal.addEventListener('click',e=>{if(e.target===modal)close()});modal.querySelector('[data-save]').onclick=()=>{const data=readPicker(modal);saveTripPatch(trip.tripId,data);apply({...trip,...data});close();onSave?.(data)}}
  async function syncCloud(){const trip=current(),client=window.ParisCloud?.client;if(!trip?.tripId||!client||isOfficialParis(trip))return;try{await window.ParisCloud.ready;const r=await client.rpc('luvia_get_trip_modules',{p_trip_id:trip.tripId});if(r.error)throw r.error;const remote=Array.isArray(r.data)?r.data[0]:r.data;if(remote&&Array.isArray(remote.modules)){const remoteSettings=remote.settings||{},localModules=Array.isArray(trip.selectedModules)?trip.selectedModules:[],remoteModules=remote.modules,localTime=Date.parse(trip.moduleSettings?._updatedAt||0)||0,remoteTime=Date.parse(remoteSettings?._updatedAt||0)||0;const localHasModules=localModules.length>0,remoteHasModules=remoteModules.length>0;const keepLocal=localHasModules&&(!remoteHasModules||!remoteTime||localTime>=remoteTime);if(keepLocal){await pushCloud(trip.tripId,{selectedModules:localModules,moduleSettings:trip.moduleSettings||{},moduleContent:trip.moduleContent||{}});apply(trip);return}if(remoteHasModules){const patch={selectedModules:remoteModules,moduleSettings:remoteSettings,moduleContent:remoteSettings.moduleContent||trip.moduleContent||{}};const existing=registry(),next=reconcileTrip({...trip,...patch});localStorage.setItem(REGISTRY_KEY,JSON.stringify(existing.some(t=>t?.tripId===trip.tripId)?existing.map(t=>t?.tripId===trip.tripId?{...t,...next}:t):[next,...existing]));localStorage.setItem(ID_KEY,JSON.stringify(next));apply(next);return}apply(trip)}}catch(error){console.info('Cloud-Modulkonfiguration nicht verfügbar:',error?.message||error)}}
  ...
  document.addEventListener('DOMContentLoaded',()=>{requestAnimationFrame(()=>{const trip=reconcileTrip(current());mountAll(trip);apply(trip);setTimeout(syncCloud,700)})});window.addEventListener('luvia:trip-changed',e=>{const active=current()||{},detail=e.detail||{},raw=detail.tripName||Array.isArray(detail.selectedModules)?detail:{...active,...(detail.changes||{}),tripId:detail.tripId||active.tripId};apply(reconcileTrip(raw))});window.addEventListener('luvia:trip-modules-changed',e=>apply(reconcileTrip(e.detail||current())));window.addEventListener('reisezeit:login-success',()=>setTimeout(()=>syncCloud(),50));
})();
