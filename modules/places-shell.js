(() => {
  ...
  async function showHub(){
    if(state.mounted) await window.LuviaModules?.unmountModule?.(state.mounted);state.mounted=null;state.view='planning';
    state.root.innerHTML='<div data-planning-foundation-host></div>';
    window.LuviaPlanningFoundation?.mount?.(state.root.querySelector('[data-planning-foundation-host]'),{surface:'places',trip:state.trip,onCatalog:showBrowse});
    return true;
  }
