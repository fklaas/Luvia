(() => {
  ...
  function render(){const p=phase();const destination=window.LuviaTripContext?.getDestination?.()||'euer Reiseziel';els.stay.classList.remove('is-visible');let title,intro,eyebrow,cards,actions='';if(p==='pretrip'){
      title=`Noch seid ihr zuhause – aber ${destination} rückt näher.`;intro=`In ${Math.max(1,Math.ceil((START-new Date())/86400000))} Tagen beginnt euer gemeinsames Abenteuer. Heute könnt ihr schon kleine Dinge erledigen, die unterwegs Ruhe schaffen.`;eyebrow=`🧳 Vorfreude auf ${destination}`;cards=[{icon:'🌤️',label:'Wetter',title:`${destination} im Blick behalten`,text:'Kurz vor der Abfahrt wird die Prognose automatisch wichtiger.'},{icon:'🗣️',label:'Sprachcoach',title:'Drei Sätze reichen schon',text:'Begrüßung, Bestellung und eine höfliche Frage üben.'},{icon:'🎒',label:'Vorbereiten',title:'Packliste & Tickets',text:'Dokumente und Reservierungen zusätzlich offline sichern.'},{icon:'❤️',label:'Vorfreude',title:'Lieblingsorte ansehen',text:'Eiffelturm, Seine und Disneyland gemeinsam entdecken.'}];actions='<button class="live-moment-btn primary" data-action="gps">📍 Live-Modus für die Reise vorbereiten</button><a class="live-moment-btn secondary" href="#sprachcoach">🗣️ Jetzt kurz üben</a>';
