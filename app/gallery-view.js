(() => {
  ...
  function editorControls(edit) {
    ...
    return `<nav class="lv-studio-tools" aria-label="Foto-Werkzeuge">
      <button type="button" data-studio-tab="look"><b>◐</b><span>Looks</span></button>
    ...
    <div class="lv-studio-drawer" data-studio-drawer>
      <header><strong data-drawer-title>Werkzeug wählen</strong><button type="button" data-drawer-close>×</button></header>
      <div class="lv-studio-pane" data-studio-pane="look"><div class="lv-filter-browser"><div>${filterButtons}</div></div></div>
      ...
      <div class="lv-studio-pane" data-studio-pane="title"><div class="lv-editor-name"><input value="" placeholder="Eigener Fototitel" data-edit-name><button type="button" data-reanalyze>⌖ Original-Metadaten neu lesen</button><button type="button" data-ai-title>✨ KI-Titel aus Bild, Zeit und Ort erkennen</button><small data-metadata-summary></small></div></div>
    </div>`;
  }
  ...
  async function openEditor(id) {
    ...
    const drawer=overlay.querySelector('[data-studio-drawer]'),drawerTitle=overlay.querySelector('[data-drawer-title]'),toolNames={look:'Looks',adjust:'Anpassen',decorate:'Kreativ',title:'Titel & KI'};
    const closeDrawer=()=>{drawer?.classList.remove('is-open');overlay.querySelectorAll('[data-studio-tab]').forEach(x=>x.classList.remove('is-active'))};
    overlay.querySelector('[data-drawer-close]')?.addEventListener('click',closeDrawer);
    overlay.querySelectorAll('[data-studio-tab]').forEach(button=>button.onclick=()=>{const key=button.dataset.studioTab,isSame=button.classList.contains('is-active')&&drawer?.classList.contains('is-open');overlay.querySelectorAll('[data-studio-tab]').forEach(x=>x.classList.toggle('is-active',x===button&&!isSame));overlay.querySelectorAll('[data-studio-pane]').forEach(x=>x.classList.toggle('is-active',!isSame&&x.dataset.studioPane===key));if(isSame)return closeDrawer();if(drawerTitle)drawerTitle.textContent=toolNames[key]||'Werkzeug';drawer?.classList.add('is-open')});
    overlay.querySelector('[data-reanalyze]')?.addEventListener('click',async e=>{const button=e.currentTarget;button.disabled=true;button.textContent='⌖ Metadaten werden gelesen …';try{item=await window.LuviaMediaCore.reanalyze(id);const resolved=item.metadata?.resolvedLocation?.name;summary.textContent=resolved?`EXIF erkannt · ${resolved}`:item.latitude!=null?'EXIF-GPS erkannt · kein konkreter Place gefunden':'Die vom Browser gelieferte Originaldatei enthält kein GPS';status('Original-Metadaten wurden neu analysiert.','ready')}catch(error){showError(error)}finally{button.disabled=false;button.textContent='⌖ Original-Metadaten neu lesen'}});
    ...
    overlay.querySelector('[data-save]').onclick=async()=>{suppressRealtimeUntil=Date.now()+1200;await window.LuviaMediaCore.update(id,{displayName:overlay.querySelector('[data-edit-name]')?.value||item.displayName||'',editSettings:state});removeOverlay();await load({silent:true,force:true});status('Fotoänderungen gespeichert.','ready')};
