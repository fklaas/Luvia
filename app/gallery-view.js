(() => {
  ...
  async function aiTitleFor(item) {
    ...
    const result = await window.LuviaOpenAIProvider.run({capability:'media.describe',tier:'fast',input:{imageUrl:url,language:'de',instruction:'Erzeuge einen kurzen, warmen und konkreten Fototitel. Keine Tageszeit-Titel und nichts erfinden.'},context:{capturedAt:item.capturedAt,latitude:item.latitude,longitude:item.longitude,dayKey:item.dayKey,...placeContextFor(item)}} ,{timeoutMs:45000});
