(() => {
  ...
  const overlayMarkup = edit => `<span class="lv-saved-overlays">${(edit.overlays||[]).map(o=>`<span class="lv-saved-overlay ${o.type==='text'?'is-text':'is-sticker'}" style="--overlay-x:${Number(o.x??50)};--overlay-y:${Number(o.y??50)};--overlay-rotation:${Number(o.rotation||0)}deg;--overlay-scale:${Number(o.scale||1)}">${esc(o.value||'')}</span>`).join('')}</span>`;
  ...
  async function openEditor(id) {
    ...
    overlay.querySelectorAll('[data-quick-sticker]').forEach(button=>button.addEventListener('click',()=>{const value=button.dataset.quickSticker;if(!value)return;state.overlays=[...(state.overlays||[]),{type:'sticker',value,x:50,y:50,scale:1,rotation:0}];apply()}));
