(() => {
  ...
  type SceneOptions = { routeCount?: number; mood?: number };
  ...
  vec3 flight(vec2 uv){vec3 c=sky(uv);float h=pow(max(0.,1.-uv.y),3.);c+=vec3(.18,.10,.03)*h;return c;}
  ...
  class MemoryRenderEngine {
    ...
    start=performance.now(); mode=0; raf=0; routeStart=0; routeDuration=7800; routeCount=7; pointer={x:.5,y:.5}; ro:ResizeObserver;
    ...
    setScene(name:SceneName,opts:SceneOptions={}){this.mode=modeFor[name]??0;this.routeCount=Math.max(3,Math.min(9,opts.routeCount||7));if(name==='flight'||name==='day')this.routeStart=performance.now();this.host.dataset.renderScene=name;}
    routePoint(t:number){const y=.58+Math.sin(t*Math.PI*2.15)*.13+Math.sin(t*Math.PI*4.)*.035;return{x:.07+t*.86,y:.22+y*.50};}
    drawRoute(now:number){const x=this.ctx,w=this.overlay.width,h=this.overlay.height,d=w/Math.max(1,this.host.clientWidth);x.clearRect(0,0,w,h);if(!(this.mode===0||this.mode===1))return;const p=Math.min(1,Math.max(0,(now-this.routeStart)/this.routeDuration));x.save();x.scale(d,d);const cw=this.host.clientWidth,ch=this.host.clientHeight;const pts=Array.from({length:90},(_,i)=>this.routePoint(i/89));x.beginPath();pts.forEach((q,i)=>{const px=q.x*cw,py=q.y*ch;i?x.lineTo(px,py):x.moveTo(px,py)});x.strokeStyle='rgba(255,255,255,.78)';x.lineWidth=2.3;x.setLineDash([10,12]);x.stroke();x.setLineDash([]);for(let i=0;i<this.routeCount;i++){const t=i/(this.routeCount-1),q=this.routePoint(t),reached=t<=p+.001;x.beginPath();x.arc(q.x*cw,q.y*ch,reached?6:4,0,Math.PI*2);x.fillStyle=reached?'rgba(255,255,255,.98)':'rgba(255,255,255,.48)';x.fill();if(reached){x.font='700 11px system-ui';x.fillStyle='rgba(18,43,58,.86)';x.fillText(String.fromCharCode(65+i),q.x*cw+10,q.y*ch-9)}}const q=this.routePoint(p),q2=this.routePoint(Math.min(1,p+.006)),ang=Math.atan2((q2.y-q.y)*ch,(q2.x-q.x)*cw);x.translate(q.x*cw,q.y*ch);x.rotate(ang);x.fillStyle='rgba(28,42,58,.95)';x.beginPath();x.moveTo(18,0);x.lineTo(-8,-5);x.lineTo(-18,-16);x.lineTo(-22,-14);x.lineTo(-14,-3);x.lineTo(-18,0);x.lineTo(-14,3);x.lineTo(-22,14);x.lineTo(-18,16);x.lineTo(-8,5);x.closePath();x.fill();x.restore();}
