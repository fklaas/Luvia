(() => {
  'use strict';
  let root,activeView='dashboard',mountedPlaces=false,overlayPortal=null,unsubscribeTrip=null,unsubscribeAuth=null,unsubscribeProfile=null,unsubscribeCollaboration=null,collaborationFrame=0,authHydration=0,lastAuthUserId=null,pendingPlaceOpen=null,todayRenderFrame=0,todayRenderTimer=0,todayLastHtml='',lastTripRenderSignature='',showSequence=0,shellInitialized=false;
  ...
  async function signedOut(){if(await window.LuviaJoinFlow?.renderIfPending?.(root,window.ParisAuth.getState(),profile()))return;if(window.LuviaGuidedJourneyEntry?.render)return window.LuviaGuidedJourneyEntry.render(root);root.innerHTML=`<main class="lv-auth lv-card"><header class="lv-auth-head"><span class="lv-logo"></span><h1>Willkommen bei Luvia</h1><p>Gemeinsam reisen. Für immer erinnern.</p></header><div class="lv-auth-slot" data-auth-slot></div></main>`;window.ParisAuthUI.renderAuthForm(root.querySelector('[data-auth-slot]'),'login')}
  ...
  function dock(t){const modules=window.LuviaModuleRegistry.enabledForTrip(t),placeIds=modules.filter(id=>['restaurants','accommodations','attractions','photo-spots','parking','charging','mobility','shopping','travel-help'].includes(id)),other=modules.filter(id=>!placeIds.includes(id));return `<div class="lv-dock-wrap"><nav class="lv-dock"><button class="lv-nav ${activeView==='dashboard'?'on':''}" data-view="dashboard">🏠 Dashboard</button>${placeIds.length?`<button class="lv-nav ${activeView==='places'?'on':''}" data-view="places">📍 Places</button>`:''}${other.map(id=>{const m=window.LuviaModuleRegistry.get(id);return `<button class="lv-nav ${activeView===id?'on':''}" data-view="${id}">${m.icon} ${esc(m.title)}</button>`}).join('')}</nav></div>`}
  ...
  async function openPlace(payload){
    ...
    if(payload.overlayOnly&&enabled){
      if(overlayPortal){try{await window.LuviaPlacesShell?.unmount?.()}catch{}overlayPortal.remove();overlayPortal=null}
      overlayPortal=document.createElement('div');overlayPortal.id='lv-place-overlay-portal';overlayPortal.setAttribute('aria-hidden','true');overlayPortal.style.cssText='position:fixed;left:-10000px;top:0;width:1200px;height:900px;visibility:hidden;pointer-events:none;overflow:hidden';document.body.appendChild(overlayPortal);
      await window.LuviaPlacesShell.mount(overlayPortal,activeTrip(),{moduleId});
      const opened=await deliverPlaceOpen(payload);
      if(!opened){await window.LuviaPlacesShell.openPlace(payload)}
      const cleanup=async()=>{window.removeEventListener('luvia:place-overlay-closed',cleanup);if(overlayPortal){try{await window.LuviaPlacesShell?.unmount?.()}catch{}overlayPortal.remove();overlayPortal=null}};
      window.addEventListener('luvia:place-overlay-closed',cleanup,{once:true});
      return;
    }
    pendingPlaceOpen=payload;
    if(enabled){await show('places',{moduleId,payload});pendingPlaceOpen=null;return;}
  ...
  async function unmountCurrent(){if(mountedPlaces){await window.LuviaPlacesShell?.unmount?.();mountedPlaces=false}}
  function transitionMeta(view){if(view==='dashboard')return{icon:'🏠',title:'Dashboard',subtitle:'Eure Reise auf einen Blick'};if(view==='places')return{icon:'📍',title:'Places',subtitle:'Orte entdecken, planen und gemeinsam erleben.'};const module=window.LuviaModuleRegistry?.get?.(view);return{icon:module?.icon||'✨',title:module?.title||'Places',subtitle:module?.description||'Orte für eure Reise werden vorbereitet.'}}
  ...
  async function show(view='dashboard',options={}){
    ...
    if(sameView&&!options.force){
      activeView=view;
      if(view==='places'&&window.LuviaPlacesShell?.active?.()){
        await window.LuviaPlacesShell.showHub?.();
        if(options.scroll!==false)window.scrollTo({top:0,behavior:'instant'});
      }
      root.querySelector('.lv-dock-wrap')?.remove();root.querySelector('.lv-shell')?.insertAdjacentHTML('beforeend',dock(t));
      return;
    }
    const sequence=++showSequence;
    await unmountCurrent();if(sequence!==showSequence)return;
    activeView=view;
    ...
    if(view==='places'){
      stage.innerHTML=wrap(view,'<div id="places-module"></div>');stage.querySelector('.lv-view-host')?.setAttribute('data-view','places');
      try{window.LuviaDestination?.refresh?.();await window.LuviaPlacesShell.mount(stage.querySelector('#places-module'),t,options);if(sequence!==showSequence)return;mountedPlaces=true;if(pendingPlaceOpen){const payload=pendingPlaceOpen;pendingPlaceOpen=null;requestAnimationFrame(()=>window.LuviaPlacesShell.openPlace(payload));}if(animate)completeTransition(stage)}catch(error){console.error(error);stage.innerHTML='<section class="lv-card" style="padding:24px"><h2>Places</h2><div class="lv-error">Das Places-Modul konnte nicht geöffnet werden.</div></section>'}
    }else{
      activeView='dashboard';stage.innerHTML=wrap('dashboard',dashboard(t));stage.querySelector('.lv-view-host')?.setAttribute('data-view','dashboard');if(animate)completeTransition(stage);
    }
    root.querySelector('.lv-dock-wrap')?.remove();root.querySelector('.lv-shell')?.insertAdjacentHTML('beforeend',dock(t));
    if(options.scroll!==false)window.scrollTo({top:0,behavior:'instant'});
  }
  function refreshDashboardGrid(){const t=activeTrip(),grid=root?.querySelector('[data-widget-grid]');if(!t||activeView!=='dashboard'||!grid)return false;grid.innerHTML=window.LuviaDashboardWidgets.render({trip:t,profile:profile(),esc});requestAnimationFrame(()=>window.LuviaTimelineCore?.bindCalendar?.(grid));return true;}
  ...
  function ready(){const t=activeTrip(),p=profile();if(!t)return noTrips();lastTripRenderSignature=tripRenderSignature(t);window.LuviaTheme.apply(t);if(root.querySelector('.lv-shell')){refreshShellHeader();return}root.innerHTML=`<div class="lv-shell"><div class="lv-ambient-memories" aria-hidden="true"><span class="lv-ambient-item lv-ambient-passport">✦</span><span class="lv-ambient-item lv-ambient-camera">◉</span><span class="lv-ambient-item lv-ambient-heart">♡</span><span class="lv-ambient-item lv-ambient-pin">⌖</span><span class="lv-ambient-item lv-ambient-ticket">◇</span><span class="lv-ambient-item lv-ambient-route">⌁</span><span class="lv-ambient-item lv-ambient-sun">✺</span><span class="lv-ambient-item lv-ambient-cloud">☁</span><span class="lv-ambient-item lv-ambient-star">✦</span></div><header class="lv-header"><span class="lv-logo"></span><div class="lv-trip"><strong>${esc(t.title||'Unsere Reise')}</strong><small>${esc(t.destination?.name||'Reiseziel offen')}</small></div><span class="lv-spacer"></span><button class="lv-icon" data-invite>＋ <span class="lv-label">Einladen</span></button><button class="lv-profile-trigger" data-profile-open="hub">${avatar(p)}<span><b>${esc(p.displayName||'Profil')}</b><small>${esc(t.title||'Reise')}</small></span></button></header><main class="lv-stage" data-stage></main></div>`;shellInitialized=true;show(activeView,{animate:true})}
  async function render(){const auth=window.ParisAuth.getState();if(auth.loading)return bootScreen();if(!auth.authenticated)return signedOut();document.documentElement.classList.remove('lv-entry-active');document.body.classList.remove('lv-entry-active');const s=snap();if(!s.loaded)return bootScreen('Reisen werden geladen …');if(window.LuviaJoinFlow?.pending?.())return window.LuviaJoinFlow.renderIfPending(root,auth,profile());if(!s.hasTrips||!s.hasActiveTrip)return noTrips();return ready()}
  async function hydrateForAuth(client,authState){const run=++authHydration;if(!authState?.authenticated){lastAuthUserId=null;render();return}const userId=authState.user?.id||null;bootScreen('Profil und Reisen werden aus der Cloud geladen …');await Promise.all([window.LuviaTripStore.loadRemote(client,{authoritative:true}),window.LuviaProfileService.load(client)]);if(run!==authHydration)return;const p=profile();if(p.activeTripId&&snap().trips.some(t=>t.id===p.activeTripId))window.LuviaTripStore.setActive(p.activeTripId);else if(snap().activeTripId&&p.activeTripId!==snap().activeTripId)await window.LuviaProfileService.setActiveTrip(snap().activeTripId);lastAuthUserId=userId;if(window.LuviaJoinFlow?.pending?.()){await window.LuviaJoinFlow.renderIfPending(root,authState,profile());return}window.LuviaRuntime.refresh();window.LuviaTheme.apply(activeTrip());window.LuviaDestination?.refresh?.();await window.LuviaCollaboration?.watchTrip?.(snap().activeTripId);activeView=['restaurants','accommodations'].includes(p.defaultView)?'places':(p.defaultView||'dashboard');render()}
  async function bootstrap(){bootScreen();try{const client=await window.LuviaSupabaseService.start();await window.LuviaJoinFlow?.start?.(client);await window.LuviaCollaboration?.start?.(client);window.LuviaTripStore.initialize();await window.LuviaDestination?.init?.();window.LuviaPWA?.register?.().catch(e=>console.warn('[LuviaPWA]',e));unsubscribeTrip?.();unsubscribeAuth?.();unsubscribeProfile?.();unsubscribeCollaboration?.();unsubscribeTrip=window.LuviaTripStore.subscribe(s=>{if(s.activeTripId&&profile().activeTripId!==s.activeTripId&&window.ParisAuth.getState().authenticated)window.LuviaProfileService.setActiveTrip(s.activeTripId).catch(console.warn);if(window.LuviaCollaboration?.snapshot?.().tripId!==s.activeTripId)window.LuviaCollaboration?.watchTrip?.(s.activeTripId).catch?.(console.warn);if(activeView==='places'&&mountedPlaces)return;const signature=tripRenderSignature(s.activeTrip);if(root?.querySelector('.lv-shell')&&activeView==='dashboard'&&signature===lastTripRenderSignature){updateDashboardWidget('destination');return}lastTripRenderSignature=signature;render()});unsubscribeProfile=window.LuviaProfileService.subscribe(()=>{window.LuviaTheme.apply(activeTrip());window.LuviaCollaboration?.heartbeat?.();refreshShellHeader();refreshDashboardGrid()});unsubscribeCollaboration=window.LuviaCollaboration?.subscribe?.(state=>{if(!state?.tripId||activeView!=='dashboard'||!root?.querySelector('.lv-shell'))return;cancelAnimationFrame(collaborationFrame);collaborationFrame=requestAnimationFrame(()=>updateDashboardWidget('members'))});unsubscribeAuth=window.ParisAuth.onChange(state=>{const uid=state?.user?.id||null;if(state?.authenticated&&uid!==lastAuthUserId)hydrateForAuth(client,state).catch(errorScreen);else if(!state?.authenticated&&lastAuthUserId!==null){lastAuthUserId=null;render()}});await hydrateForAuth(client,window.ParisAuth.getState())}catch(error){console.error('[LuviaApp]',error);errorScreen(error)}}
  ...
  function bind(){root.addEventListener('click',e=>{const suggestion=e.target.closest('[data-today-suggestion-add]');if(suggestion){e.preventDefault();e.stopPropagation();return addTodaySuggestion(suggestion)}const placeTarget=e.target.closest('[data-place-open]');if(placeTarget){e.preventDefault();return openPlace(placeOpenPayload(placeTarget)).catch(console.error)}if(e.target.closest('[data-retry]'))return bootstrap();if(e.target.closest('[data-create]'))return window.LuviaTripCreator.open();if(e.target.closest('[data-signout]'))return window.ParisAuth.signOut();if(e.target.closest('[data-join-code]'))return window.LuviaJoinFlow?.openCodeEntry?.();if(e.target.closest('[data-invite]'))return openDialog('trip.invite',activeTrip());if(e.target.closest('[data-edit]'))return openDialog('trip.edit',activeTrip());const po=e.target.closest('[data-profile-open]');if(po)return window.LuviaProfileFoundation.open(po.dataset.profileOpen||'hub');const view=e.target.closest('[data-view]')?.dataset.view;if(view){e.preventDefault();if(view==='places'&&activeView==='places'&&window.LuviaPlacesShell){return window.LuviaPlacesShell.showHub?.().then?.(()=>window.scrollTo({top:0,behavior:'instant'}))}return show(view)}})}
  document.addEventListener('visibilitychange',()=>{if(document.visibilityState!=='visible')return;window.LuviaDestination?.refresh?.();const id=activeTrip()?.id||activeTrip()?.tripId;if(id){window.LuviaTripPlaceData?.hydrate?.(id).catch?.(()=>{});window.LuviaTimelineCore?.hydrate?.(id).catch?.(()=>{})}});window.addEventListener('pageshow',event=>{if(event.persisted){const id=activeTrip()?.id||activeTrip()?.tripId;if(id)window.LuviaTimelineCore?.hydrate?.(id).catch?.(()=>{})}});window.addEventListener('luvia:members-changed',()=>updateDashboardWidget('members'));window.addEventListener('luvia:restaurant-intelligence-changed',()=>updateDashboardWidget('restaurantIntelligence'));window.addEventListener('luvia:schedule-intelligence-changed',()=>{if(activeView==='dashboard'&&root?.querySelector('.lv-shell'))window.LuviaTodayIntelligence?.refresh?.({refreshSchedule:false}).catch?.(()=>{})});window.addEventListener('luvia:today-intelligence-changed',()=>window.LuviaLiveDayCompanion?.refresh?.({refreshToday:false}).catch?.(()=>{}));window.addEventListener('luvia:place-plan-changed',()=>{window.LuviaScheduleIntelligence?.refresh?.({force:true,skipThrottle:true}).then(()=>window.LuviaTodayIntelligence?.refresh?.({refreshSchedule:false})).catch?.(()=>{});if(activeView==='dashboard'&&root?.querySelector('.lv-shell'))updateTodayWidget()});window.addEventListener('luvia:timeline-cloud-changed',()=>{if(activeView==='dashboard'&&root?.querySelector('.lv-shell')){updateDashboardWidget('today');requestAnimationFrame(()=>window.LuviaTimelineCore?.bindCalendar?.(root))}});window.addEventListener('luvia:live-day-changed',()=>{if(activeView==='dashboard'&&root?.querySelector('.lv-shell'))updateTodayWidget()});window.addEventListener('luvia:theme-changed',event=>{const accent=event.detail?.palette?.accent;if(!accent)return;document.documentElement.style.setProperty('--module-accent',accent);document.querySelectorAll('#restaurants-module,#accommodations-module,.lv-module-host,.lv-dashboard').forEach(el=>{el.style.setProperty('--trip-accent',accent);el.style.setProperty('--module-accent',accent);el.style.setProperty('--rv2-accent',accent)})});
  window.addEventListener('luvia:open-place-request',e=>openPlace(e.detail).catch(console.error));window.addEventListener('luvia:in-window-data-changed',()=>{if(activeView==='dashboard'&&root?.querySelector('.lv-shell')){updateDashboardWidget('today');requestAnimationFrame(()=>window.LuviaTimelineCore?.bindCalendar?.(root))}});window.addEventListener('luvia:trip-modules-changed',()=>{const t=activeTrip();if(!t)return;const enabled=window.LuviaModuleRegistry.enabledForTrip(t);const placeEnabled=enabled.some(id=>['restaurants','accommodations'].includes(id));if(activeView==='places'&&!placeEnabled)activeView='dashboard';if(root?.querySelector('.lv-shell'))show(activeView,{force:true,animate:false,scroll:false}).catch(console.error)});window.addEventListener('luvia:place-overlay-closed',()=>{});window.LuviaApp=Object.freeze({version:'13.5.1.3',bootstrap,render,show,openPlace,activeView:()=>activeView});window.addEventListener('DOMContentLoaded',()=>{root=document.getElementById('app');bind();bootstrap()},{once:true});
})();
