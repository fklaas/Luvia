(() => {
  ...
  async function show(view='dashboard'){
    ...
    if(view==='restaurants'){
      ...
      try{window.LuviaDestination?.refresh?.();window.LuviaModules.mountModule('restaurants',t);mountedRestaurant=true}catch(error){console.error(error);stage.innerHTML='<section class="lv-card" style="padding:24px"><h2>Restaurants</h2><div class="lv-error">Das Restaurantmodul konnte nicht geöffnet werden.</div></section>'}
  ...
  window.LuviaApp=Object.freeze({version:'11.2.11',bootstrap,render,show});
