(() => {
  'use strict';
  let root,activeView='dashboard',mountedPlaces=false,lastRenderedTripId=null,tripSwitchToken=0,overlayPortal=null,unsubscribeTrip=null,unsubscribeAuth=null,unsubscribeProfile=null,unsubscribeCollaboration=null,collaborationFrame=0,authHydration=0,lastAuthUserId=null,pendingPlaceOpen=null,todayRenderFrame=0,todayRenderTimer=0,todayLastHtml='',lastTripRenderSignature='',showSequence=0,shellInitialized=false,bootComplete=false,subscriptionsBound=false;
  ...
  async function openPlace(payload){
    ...
    if(payload.overlayOnly&&enabled){
      ...
      const typeLabel=payload.type==='accommodation'?'Unterkunft':payload.type==='restaurant'?'Restaurant':payload.type==='attraction'?'Aktivität':'Place';
  ...
  async function show(view='dashboard',options={}){
    ...
    if(view==='places'){
      stage.innerHTML=wrap(view,'<div id="places-module"></div>');stage.querySelector('.lv-view-host')?.setAttribute('data-view','places');stage.querySelector('.lv-view-host')?.setAttribute('data-trip-id',requestedTripId);
      ...
      activeView='dashboard';stage.innerHTML=wrap('dashboard',dashboard(t));stage.querySelector('.lv-view-host')?.setAttribute('data-view','dashboard');stage.querySelector('.lv-view-host')?.setAttribute('data-trip-id',requestedTripId);lastRenderedTripId=requestedTripId;requestAnimationFrame(()=>window.LuviaTimelineCore?.bindCalendar?.(stage));if(animate)completeTransition(stage);
    }
    root.querySelector('.lv-dock-wrap')?.remove();root.querySelector('.lv-shell')?.insertAdjacentHTML('beforeend',dock(t));
