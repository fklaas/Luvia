(() => {
  ...
  async function hydrateForAuth(client,authState){const run=++authHydration;if(!authState?.authenticated){lastAuthUserId=null;bootComplete=true;await render();await window.LuviaBootCoordinator.reveal();return}const userId=authState.user?.id||null;await window.LuviaBootCoordinator.boot(client);if(run!==authHydration)return;const p=profile();lastAuthUserId=userId;activeView=['restaurants','accommodations'].includes(p.defaultView)?'places':(p.defaultView||'dashboard');window.LuviaRuntime.refresh();window.LuviaTheme.apply(activeTrip());window.LuviaDestination?.refresh?.();bootComplete=true;await render();await window.LuviaBootCoordinator.reveal();const activeId=snap().activeTripId;if(activeId){await window.LuviaCollaboration?.start?.(client);window.LuviaCollaboration?.watchTrip?.(activeId).catch?.(console.warn)}}
  async function bootstrap(){bootScreen();try{const client=await window.LuviaSupabaseService.start();await window.LuviaJoinFlow?.start?.(client);window.LuviaPWA?.register?.().catch(e=>console.warn('[LuviaPWA]',e));if(!subscriptionsBound){subscriptionsBound=true;unsubscribeTrip=window.LuviaTripStore.subscribe(s=>{if(!bootComplete)return;if(s.activeTripId&&profile().activeTripId!==s.activeTripId&&window.ParisAuth.getState().authenticated)window.LuviaProfileService.setActiveTrip(s.activeTripId).catch(console.warn);if(window.LuviaCollaboration?.snapshot?.().tripId!==s.activeTripId)window.LuviaCollaboration?.watchTrip?.(s.activeTripId).catch?.(console.warn);if(activeView==='places'&&mountedPlaces)return;const signature=tripRenderSignature(s.activeTrip);if(root?.querySelector('.lv-shell')&&activeView==='dashboard'&&signature===lastTripRenderSignature){updateDashboardWidget('destination');return}lastTripRenderSignature=signature;render()});unsubscribeProfile=window.LuviaProfileService.subscribe(()=>{if(!bootComplete)return;window.LuviaTheme.apply(activeTrip());window.LuviaCollaboration?.heartbeat?.();refreshShellHeader();refreshDashboardGrid()});unsubscribeCollaboration=window.LuviaCollaboration?.subscribe?.(state=>{if(!bootComplete||!state?.tripId||activeView!=='dashboard'||!root?.querySelector('.lv-shell'))return;cancelAnimationFrame(collaborationFrame);collaborationFrame=requestAnimationFrame(()=>updateDashboardWidget('members'))});unsubscribeAuth=window.ParisAuth.onChange(state=>{const uid=state?.user?.id||null;if(state?.authenticated&&uid!==lastAuthUserId){window.LuviaBootCoordinator.reset();bootComplete=false;hydrateForAuth(client,state).catch(errorScreen)}else if(!state?.authenticated&&lastAuthUserId!==null){lastAuthUserId=null;bootComplete=true;render()}})}await hydrateForAuth(client,window.ParisAuth.getState())}catch(error){console.error('[LuviaApp]',error);bootComplete=true;errorScreen(error)}}
