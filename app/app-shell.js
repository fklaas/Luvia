(() => {
  'use strict';
  let root,activeView='dashboard',mountedPlaces=false,lastRenderedTripId=null,tripSwitchToken=0,overlayPortal=null,unsubscribeTrip=null,unsubscribeAuth=null,unsubscribeProfile=null,unsubscribeCollaboration=null,collaborationFrame=0,authHydration=0,lastAuthUserId=null,hydratedAuthUserId=null,pendingPlaceOpen=null,todayRenderFrame=0,todayRenderTimer=0,todayLastHtml='',lastTripRenderSignature='',showSequence=0,shellInitialized=false,bootComplete=false,subscriptionsBound=false;
  ...
  function dock(t){const modules=window.LuviaModuleRegistry.enabledForTrip(t),placeIds=modules.filter(id=>['restaurants','accommodations','attractions','photo_spots','parking','charging','mobility','shopping','travel-help'].includes(id)),other=modules.filter(id=>!placeIds.includes(id));return `<div class="lv-dock-wrap"><nav class="lv-dock"><button class="lv-nav ${activeView==='dashboard'?'on':''}" data-view="dashboard">🏠 Dashboard</button>${placeIds.length?`<button class="lv-nav ${activeView==='places'?'on':''}" data-view="places">📍 Places</button>`:''}${other.map(id=>{const m=window.LuviaModuleRegistry.get(id);return `<button class="lv-nav ${activeView===id?'on':''}" data-view="${id}">${m.icon} ${esc(m.title)}</button>`}).join('')}</nav></div>`}

  const moduleForPlaceType=type=>({restaurant:'restaurants',accommodation:'accommodations',attraction:'attractions',photo_spot:'photo_spots',activity:'activities',shopping:'shopping',nature:'nature',family:'family',mobility:'mobility',transit:'transit',custom:'custom-places'}[type]||null);
