...
async function renderHome(){
  ...
  const effectiveCards=cards.filter(c=>!(c.card_type==='reaction'&&cards.some(v=>String(v.cluster_id)===String(c.cluster_id)&&String(v.author_id)===String(c.author_id)&&v.card_type==='vibe')));
  const meId=me().id;
  ...
  host.querySelectorAll('[data-title-propose]').forEach(b=>b.onclick=async e=>{e.stopPropagation();const title=prompt('Wie würdest du diesen Kartenstapel nennen? Dein Vorschlag wird mit deinem Profil gespeichert.','');if(!title?.trim())return;b.disabled=true;try{await window.LuviaMemoryCards.saveTitleProposal(b.dataset.titlePropose,title);await renderHome()}catch(error){alert(error.message||error)}finally{b.disabled=false}});
...
function renderStack(key,items,members,index){
  ...
  const momentLabel=state?.chosen_title||stackAutoTitle(cluster,homeState?.src?.media||[]);
  ...
  return `<div class="mc-deck-wrap"><button class="mc-deck" data-stack="${esc(key)}" style="--deck-rot:${rot}deg;--deck-lift:${lift}px;--deck-accent:${esc(accent)}">
    ${layers}
    <span class="mc-deck-front tone-${cardTone(hero.card_type)}" style="--person-color:${esc(accent)}"><span class="mc-deck-photo" data-card-media="${esc(hero.media_id||'')}">${hero.media_id?'':`<b>${typeIcon(hero.card_type)}</b>`}</span><span class="mc-deck-info"><span class="mc-deck-meta"><small>${items.length} ${items.length===1?'Card':'Cards'} · ${people.length} ${people.length===1?'Stimme':'Stimmen'}</small><span class="mc-voices">${voices}</span></span><strong>${esc(momentLabel)}</strong><i>${heroCount} Foto${heroCount===1?'':'s'} · ${storyCount} Story · ${signalCount} Signal${signalCount===1?'':'e'} · ${reviewReady?'bereit zur Priorisierung':`${reviewed}/${items.length} gemeinsam geprüft`}</i></span></span>
  </button>${cluster?`<div class="mc-deck-curation"><button type="button" data-title-propose="${esc(clusterId)}">Titel vorschlagen${proposals.length?` · ${proposals.length}`:''}</button>${isTripOwner()?`<button type="button" class="danger" data-stack-dissolve="${esc(clusterId)}">Stapel auflösen</button>`:''}</div>`:''}</div>`
}
...
window.LuviaAlbumsView=Object.freeze({version:VERSION,build:BUILD,mount,render:renderHome,experience:'memory-curation-foundation',model:'cards -> decks -> moments -> journeys -> studio'});
})();
