(() => {
'use strict';
const VERSION='4.37.1',BUILD='13.37.1';
...
async function renderHome(){
  ...
  let src,cards,members,curation={states:{},proposals:{}},reviewSummary={byCard:{},reviewers:0};
  try{[src,cards,members]=await Promise.all([window.LuviaMemoryJourneys.source(),window.LuviaMemoryCards.list(),window.LuviaMemoryCards.members()]);[curation,reviewSummary]=await Promise.all([window.LuviaMemoryCards.stackCuration?.((src.clusters||[]).map(c=>c.id))||curation,window.LuviaMemoryCards.albumReviewSummary?.(cards.map(c=>c.id))||reviewSummary])}catch(e){host.innerHTML=`<section class="mc-home"><div class="mc-error"><h2>Memory Cards sind noch nicht bereit</h2><p>${esc(e.message||e)}</p></div></section>`;return}
  const clusters=(src.clusters||[]).filter(c=>Array.isArray(c.mediaIds)&&c.mediaIds.length);
  const effectiveCards=cards; // 13.37.1: legacy reactions stay visible, but are rendered as richer supporting signals instead of being silently dropped.
  const meId=me().id;
  ...
  homeState={src,cards:effectiveCards,members,clusters,grouped,curation,reviewSummary};
...
function renderStack(key,items,members,index){
  ...
  </button>${cluster?`<div class="mc-deck-curation"><button type="button" data-title-propose="${esc(clusterId)}">Titel vorschlagen${proposals.length?` · ${proposals.length}`:''}</button>${reviewReady&&candidates?`<button type="button" class="primary" data-vote-open="${esc(clusterId)}">Lieblingsmomente wählen</button>`:''}${isTripOwner()?`<button type="button" class="danger" data-stack-dissolve="${esc(clusterId)}">Stapel auflösen</button>`:''}</div>`:''}</div>`
}
...
async function openVotingModal(clusterId){const key=`cluster:${clusterId}`,items=homeState?.grouped?.get(key)||[];const candidates=items.filter(c=>(homeState?.reviewSummary?.byCard?.[String(c.id)]?.included||0)>0);if(!candidates.length)return;const budget=voteBudget(candidates.length),saved=await window.LuviaMemoryCards.albumVotes?.(clusterId,candidates.map(c=>c.id))||{};const votes={...saved};const m=curationModal(`<button class="mc-modal-x" data-modal-close>×</button><small>EURE LIEBLINGSMOMENTE</small><h2>Welche Karten sollen besonders zählen?</h2><p>Du hast <b data-budget-left>${budget}</b> Punkte. Vergib pro Karte bis zu 3 – je wichtiger sie dir ist, desto mehr.</p><div class="mc-vote-list">${candidates.map(c=>`<article data-vote-card="${esc(c.id)}"><span class="mc-vote-icon">${c.media_id?'📸':typeIcon(c.card_type)}</span><div><strong>${esc(c.content||typeName(c.card_type))}</strong><small>${esc(whoName(c,homeState.members))} · ${esc(typeName(c.card_type))}</small></div><div class="mc-vote-points">${[0,1,2,3].map(n=>`<button data-points="${n}" class="${Number(votes[String(c.id)]||0)===n?'is-selected':''}">${n}</button>`).join('')}</div></article>`).join('')}</div><div class="mc-modal-actions"><button data-modal-close>Später</button><button class="is-primary" data-save-votes>Auswahl speichern</button></div>`);const left=()=>budget-Object.values(votes).reduce((a,b)=>a+Number(b||0),0),refresh=()=>{m.root.querySelector('[data-budget-left]').textContent=String(Math.max(0,left()));m.root.querySelectorAll('[data-vote-card]').forEach(row=>row.querySelectorAll('[data-points]').forEach(b=>b.classList.toggle('is-selected',Number(votes[row.dataset.voteCard]||0)===Number(b.dataset.points))))};m.root.querySelectorAll('[data-vote-card]').forEach(row=>row.querySelectorAll('[data-points]').forEach(b=>b.onclick=()=>{const id=row.dataset.voteCard,n=Number(b.dataset.points),current=Number(votes[id]||0);if(left()+current-n<0){b.classList.add('limit-hit');setTimeout(()=>b.classList.remove('limit-hit'),300);return}votes[id]=n;refresh()}));m.root.querySelector('[data-save-votes]').onclick=async e=>{const b=e.currentTarget;b.disabled=true;try{await window.LuviaMemoryCards.saveAlbumVotes(clusterId,votes,budget);m.close();await renderHome()}catch(err){b.disabled=false;alert(err.message||err)}};refresh()}
...
function renderLooseCard(c,members,i,mode='deck'){
  ...
  const reaction=c.reaction?`<strong>${esc(c.reaction)}</strong>`:(c.card_type==='reaction'?`<strong>${esc(c.content||'✨')}</strong>`:'');
...
window.LuviaAlbumsView=Object.freeze({version:VERSION,build:BUILD,mount,render:renderHome,experience:'memory-curation-ux-voting-entry',model:'cards -> decks -> moments -> journeys -> studio'});
})();
