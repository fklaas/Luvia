...
for(const token of ["domain:'places'",'LuviaGuidedDiscovery','hideBrowse:true','data-guided-open-catalog','LuviaUserPreferences?.get'])assert(places.includes(token),`Places guided flow missing ${token}`);
for(const token of ["domain:'move'",'LuviaGuidedDiscovery','hideBrowse:true','data-guided-open-catalog','LuviaUserPreferences?.get'])assert(move.includes(token),`Move guided flow missing ${token}`);
...
for(const token of ['gds-cloud-shape','gds-flight-track','gds-plane','prefers-reduced-motion','gds-profile-hint'])assert(css.includes(token),`guided visual language missing ${token}`);
for(const token of ['requestAnimationFrame','touchstart','updateMultiSelectionUI','is-confirmed','Mehr entdecken','sessionStorage','Cloud-Speicherung fehlgeschlagen','suspendUnderlyingSurface'])assert(guided.includes(token),`guided sequence behavior missing ${token}`);
