...
for(const token of ['cycling.health','cycling.search','cycling.details','OVERPASS_API_URL','overpass-api.de','^(bicycle|mtb)$','mtb:scale','geometrySegments','© OpenStreetMap-Mitwirkende'])assert(cycling.includes(token),`cycling gateway token missing: ${token}`);
