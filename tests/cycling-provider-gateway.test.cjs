...
for(const token of ['cycling.health','cycling.search','cycling.search.routes','cycling.search.trails','cycling.details','OVERPASS_API_URL','overpass-api.de','routeRelationsQuery','mtb:scale','geometrySegments','© OpenStreetMap-Mitwirkende'])assert(cycling.includes(token),`cycling gateway token missing: ${token}`);
