...
const document={
 currentScript:{src:'https://example.test/core/places/place-type-definitions.js?v=13.19.1'},
...
assert.strictEqual(window.LuviaPlaceTypeContracts.version,'4.19.1','full contract did not upgrade the bootstrap');
...
for(const [type,key] of Object.entries(planned)){
 ...
 assert.strictEqual(contract.contractVersion,'4.19.1',`${type} contract version was not upgraded`);
 ...
 assert.strictEqual(registry.contractVersion,'4.19.1',`${type} registry metadata did not refresh`);
...
assert.strictEqual(mobility.contractVersion,'4.19.1','Move contract version was not upgraded');
