/* Build 13.8.1.1 – static architecture regression test */
const fs=require('fs');
const assert=require('assert');
const files=['modules/restaurants-v2/restaurant-module.js','modules/accommodations/accommodation-module.js','modules/attractions/attraction-module.js','modules/photo-spots/photo-spot-module.js'];
...
const index=fs.readFileSync('index.html','utf8');assert(index.includes('place-runtime-store.js'));assert(index.includes('place-command-service.js'));const placeUi=fs.readFileSync('core/places/place-ui.js','utf8');assert(placeUi.includes('function insightGrid'),'global insight renderer missing');
