...
function parkingSummary(options={}){const labels=[];if(options.freeParkingLot)labels.push('Kostenloser Parkplatz');if(options.paidParkingLot)labels.push('Gebührenpflichtiger Parkplatz');if(options.freeGarageParking)labels.push('Kostenloses Parkhaus');if(options.paidGarageParking)labels.push('Gebührenpflichtiges Parkhaus');if(options.freeStreetParking)labels.push('Kostenlose Straßenplätze');if(options.paidStreetParking)labels.push('Gebührenpflichtige Straßenplätze');if(options.valetParking)labels.push('Valet-Parking');return labels.join(', ')}
function chargeSummary(options={}){const count=Number(options.connectorCount||options.totalConnectorCount||options.count);const parts=[];if(Number.isFinite(count)&&count>0)parts.push(`${count} Anschlüsse`);const connectors=Array.isArray(options.connectorAggregation)?options.connectorAggregation:Array.isArray(options.connectors)?options.connectors:[];const labels=connectors.map(x=>x.type||x.connectorType||x.maxChargeRateKw&&`${x.maxChargeRateKw} kW`).filter(Boolean);if(labels.length)parts.push([...new Set(labels)].slice(0,4).join(', '));return parts.join(' · ')}
function render(place={}){const rows=[];const park=parkingSummary(place.parkingOptions||{}),charge=chargeSummary(place.evChargeOptions||{});if(park)rows.push(`<div class="luv-place-provider-row"><span>🅿️ Parken</span><strong>${esc(park)}</strong></div>`);if(charge)rows.push(`<div class="luv-place-provider-row"><span>⚡ Ladeoptionen</span><strong>${esc(charge)}</strong></div>`);for(const [key,label,icon] of defs){const raw=normalized(place,key);const val=value(key,raw);if(!val)continue;rows.push(row(key,label,icon,raw,val))}
