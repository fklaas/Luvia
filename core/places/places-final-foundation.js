(() => {
  ...
  function score(place){const known=state.known.get(providerId(place));let s=0;s+=Math.min(25,Number(place.rating||0)*5);s+=Math.min(15,Math.log10(Math.max(1,Number(place.userRatingCount||0)))*5);if(place.currentOpeningHours?.openNow===true||place.openNow===true)s+=10;if(place.formattedAddress||place.address)s+=5;if(place.location||place.latitude)s+=5;if(known?.planned)s-=18;if(/visited|checked_in|checked_out/.test(known?.status||''))s-=15;const rel=window.LuviaGlobalPlaceContracts?.relevance?.(place,state.activeGoal?.text||'',state.activeGoal?.category||'',currentPreferences())||{score:0,reasons:[]};place._luviaReasons=rel.reasons||[];place._luviaIntent=rel.intent||null;s+=Number(rel.score||0);return s}
