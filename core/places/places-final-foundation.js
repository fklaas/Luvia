(() => {
  ...
  function profilePayload(){const prefs=currentPreferences(),raw=JSON.stringify(prefs).toLowerCase(),members=JSON.stringify(participants()).toLowerCase();return{vegetarian:/vegetar/.test(raw),vegan:/vegan/.test(raw),withChild:/baby|kind|livi/.test(members),stroller:/kinderwagen|buggy/.test(raw),preferences:[...(prefs.likes||[]),...(prefs.travelPreferences?.likes||[]),...(prefs.preferences||[])].map(clean).filter(Boolean).slice(0,12)}}
  ...
  function searchQueries(goal){const dest=destination(state.trip);return window.LuviaGlobalPlaceContracts?.queryCascade?.(goal,dest,currentPreferences())||[`${goal.text||''} ${dest}`.trim()]}
  ...
  function score(place){const known=state.known.get(providerId(place));let s=0;s+=Math.min(25,Number(place.rating||0)*5);s+=Math.min(15,Math.log10(Math.max(1,Number(place.userRatingCount||0)))*5);if(place.currentOpeningHours?.openNow===true||place.openNow===true)s+=10;if(place.formattedAddress||place.address)s+=5;if(place.location||place.latitude)s+=5;if(known?.planned)s-=18;if(/visited|checked_in|checked_out/.test(known?.status||''))s-=15;const rel=window.LuviaGlobalPlaceContracts?.relevance?.(place,state.activeGoal?.text||'',state.activeGoal?.category||'',currentPreferences())||{score:0,reasons:[]};place._luviaReasons=[...(rel.reasons||[])];const sources=Object.keys(place.providerRefs||{});if(sources.length>1)place._luviaReasons.push('Von mehreren Places-Quellen bestätigt.');else if(sources.includes('foursquare'))place._luviaReasons.push('Über Foursquare als passender Spezialtreffer gefunden.');const profile=profilePayload();if(profile.vegetarian&&state.activeGoal?.category==='food')place._luviaReasons.push(place.features?.servesVegetarianFood===true?'Vegetarische Eignung ist ausdrücklich belegt.':'Der Profil-Kompass priorisiert vegetarische Optionen; die konkrete Eignung sollte im Detail geprüft werden.');place._luviaIntent=rel.intent||null;s+=Number(rel.score||0);return s}
