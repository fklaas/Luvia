(() => {
  ...
  function searchQueries(goal){const dest=destination(state.trip);return window.LuviaGlobalPlaceContracts?.queryCascade?.(goal,dest)||[`${goal.text||''} ${dest}`.trim()]}
  function validForCategory(place,category){if(window.LuviaGlobalPlaceContracts?.accepts)return window.LuviaGlobalPlaceContracts.accepts(place,category,state.activeGoal?.text||'');return Boolean(providerId(place)&&clean(place?.name).length>=2)}
  ...
  async function search(goal=state.activeGoal){
    ...
    try{
      ...
      let places=[];for(const query of searchQueries(goal)){if(seq!==state.sequence)return false;const response=await window.LuviaPlaceEntities.searchPlaces({tripId:tripId(state.trip),type:def.type,includedType:def.includedType,query,maxResultCount:14,strictDestination:true});places.push(...(response?.data?.places||[]).filter(p=>!state.rejected.has(providerId(p))).filter(p=>validForCategory(p,goal.category)));if(places.length>=10)break}if(seq!==state.sequence)return false;
