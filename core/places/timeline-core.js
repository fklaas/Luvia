...
window.addEventListener('luvia:trip-changed',e=>{const id=e.detail?.tripId||e.detail?.id||activeTripId();if(!id)return;hydrate(id).then(()=>{if(activeTripId()===id)subscribeRealtime(id)}).catch(()=>{})});window.addEventListener('luvia:place-plan-changed',()=>hydrate().catch(()=>{}));window.addEventListener('luvia:place-visit-changed',()=>hydrate().catch(()=>{}));
window.LuviaTimelineCore=Object.freeze({version:VERSION,init,hydrate,record,removeEntry,clearEntries,snapshot,list,entriesForDate,renderCalendar,bindCalendar,popup,openPlanningEditor,editEntry,subscribe,diagnostics});
})();