...
async function openPhotoMemory(e,node){
 ...
 try{
  ...
  if(!selected.length){collage.classList.remove('is-loading');collage.innerHTML='<span>Keine Fotos mehr mit diesem Moment verknüpft.</span>';return modal}
  collage.innerHTML=selected.map(x=>`<button type="button" data-memory-photo="${x.id}" aria-label="${String(x.displayName||'Foto')}"><img alt="${String(x.displayName||'Reisefoto')}" loading="eager"></button>`).join('');collage.classList.remove('is-loading');
  for(const item of selected){const url=await window.LuviaMediaCore.signedUrl(item,1800).catch(()=>null),button=collage.querySelector(`[data-memory-photo="${CSS.escape(item.id)}"]`),img=button?.querySelector('img');if(url&&img){img.src=url;button.onclick=()=>{const viewer=document.createElement('div');viewer.className='lv-photo-overlay';viewer.innerHTML=`<section class="lv-photo-dialog"><button data-close>×</button><div class="lv-photo-large"><img src="${url}" alt="${String(item.displayName||'Reisefoto')}"></div><footer><div><strong>${String(item.displayName||'Reisefoto')}</strong><small>${fmtDate(item.capturedAt)} · ${fmtTime(item.capturedAt)}</small></div></footer></section>`;document.body.appendChild(viewer);viewer.querySelector('[data-close]').onclick=()=>viewer.remove();viewer.onclick=event=>{if(event.target===viewer)viewer.remove()}}}else if(button)button.innerHTML='<span>Vorschau nicht verfügbar</span>'}
