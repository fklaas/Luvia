(() => {
  'use strict';
  const VERSION='4.29.3.1', BUILD='13.29.3.1';
  ...
  const kind=m=>{const n=(m.displayName||m.originalName||'').toLowerCase(),meta=m.metadata||{},ratio=m.width&&m.height?m.width/m.height:null;if(meta.mediaKind)return meta.mediaKind;if(/screenshot|bildschirmfoto|screen[_ -]?shot/.test(n))return'screenshot';if(/ticket|boarding|rechnung|receipt|qr|document|scan/.test(n))return'document';if(ratio&&ratio<.62)return'screenshot';return'photo'};
