(() => {
  'use strict';
  const VERSION='4.29.1',BUILD='13.29.1',BUCKET='luvia-media',channels=new Map();
  ...
  async function listByIds(ids=[]){const unique=[...new Set((ids||[]).map(String).filter(Boolean))];if(!unique.length)return[];const{client,tripId}=await context();const chunks=[];for(let i=0;i<unique.length;i+=100)chunks.push(unique.slice(i,i+100));const rows=[];for(const chunk of chunks){const r=await client.from('media').select('*').eq('trip_id',tripId).in('id',chunk).neq('status','deleted');if(r.error)throw r.error;rows.push(...(r.data||[]))}const map=new Map(rows.map(r=>[String(r.id),entity(r)]));return unique.map(id=>map.get(id)).filter(Boolean)}
  ...
  window.LuviaMediaCore=Object.freeze({version:VERSION,build:BUILD,bucket:BUCKET,getContext:context,list,listByIds,get,upload,update,reanalyze,toggleFavorite,listPolaroids,setPolaroid,linkPlace,remove,signedUrl,signedOriginalUrl,saveRenderedPreview,subscribe,diagnostics,rowToEntity:entity});
})();
