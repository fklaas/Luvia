(() => {
  ...
  function save(coords){state.location={latitude:coords.latitude,longitude:coords.longitude,accuracy:coords.accuracy,updatedAt:Date.now()};state.requesting=false;state.error=null;state.permission='granted';localStorage.setItem(LOCATION_PREF,'1');localStorage.setItem(LOCATION_CACHE,JSON.stringify(state.location));emit('location')}
  ...
  window.LuviaTravelContext=Object.freeze({version:'1.0.0',snapshot,subscribe(fn){listeners.add(fn);fn(snapshot());return()=>listeners.delete(fn)},requestLocation,clearLocation,distanceTo(target){return haversine(state.location,target)},haversine,eventCountdown,phase,tripDay,daysUntil});
})();
