(() => {
  ...
  function markStorageUnavailable(error) {
    ...
    console.warn('Galerie: IndexedDB nicht verfügbar, verwende Sitzungsspeicher.', error?.message || error || 'unbekannter Fehler');
  }
  ...
  function openDB() {
    return new Promise((resolve, reject) => {
      ...
      request.onblocked = () => {
        if (!settled) reject(new Error('IndexedDB ist durch eine ältere App-Version blockiert.'));
      };
    });
  }
  ...
  async function uniqueId(file) {
    // Mobile Browser liefern Kamera-Dateien teils ohne verlässlichen MIME-Typ.
    // Für die Dublettenprüfung werden nur kleine Dateiausschnitte gehasht, damit
    // große Handyfotos nicht komplett in den Arbeitsspeicher geladen werden.
    if (crypto.subtle) {
      ...
      const last = tailStart ? await file.slice(tailStart).arrayBuffer() : new ArrayBuffer(0);
      const meta = new TextEncoder().encode(`${file.name || 'camera'}|${file.size}|${file.lastModified || 0}`);
      const joined = new Uint8Array(first.byteLength + last.byteLength + meta.byteLength);
      joined.set(new Uint8Array(first), 0);
      joined.set(new Uint8Array(last), first.byteLength);
      joined.set(meta, first.byteLength + last.byteLength);
  ...
  async function importFiles(files) {
    ...
    if (!images.length) {
      toast('Bitte wählt ein Foto oder Bild aus.');
      return;
    }
    ...
    try {
      for (const file of images) {
        ...
        try {
          ...
          const item = {
            ...
            syncPending: Boolean(window.ParisSync?.gallery)
          };

          // Sofort lokal speichern und anzeigen. So verschwindet ein Kamerafoto
          // in Chrome nicht, nur weil Mobilfunk oder Cloud-Upload kurz stocken.
          await upsertLocalPhoto(item);
          render();
          added++;
          if (!state.storageAvailable && !state.storageWarningShown) {
            state.storageWarningShown = true;
            console.warn('Das Foto bleibt in dieser Sitzung sichtbar und wird direkt mit Supabase synchronisiert.');
          }

          if (window.ParisSync?.gallery) {
            try {
              await window.ParisSync.gallery.upload(item);
              item.syncPending = false;
              item.storagePath = item.storagePath || null;
              await put(item);
            } catch (cloudError) {
              cloudPending++;
              item.syncPending = true;
              await put(item);
              console.warn('Galerie-Upload wird später erneut versucht:', cloudError);
            }
          }
        } finally {
      ...
      render();
      if (cloudPending) {
        toast(`${added} Foto${added === 1 ? '' : 's'} einsortiert · ${cloudPending} wartet auf Synchronisation`);
      } else {
        toast(added ? `${added} Foto${added === 1 ? '' : 's'} automatisch einsortiert` : 'Keine neuen Fotos gefunden');
      }
    } catch (error) {
      console.error(error);
      toast(added ? 'Foto gespeichert – weitere Bilder konnten nicht verarbeitet werden.' : 'Das Foto konnte nicht gespeichert werden.');
    } finally {
      state.busy = false;
      ...
      for (const local of [...state.photos]) {
        if (!remoteById.has(local.id)) {
          if (local.syncPending) {
            try {
              await window.ParisSync.gallery.upload(local);
              local.syncPending = false;
              await put(local);
              changed = true;
            } catch (retryError) {
              // Offline oder Upload noch nicht möglich: lokale Aufnahme behalten.
              console.warn('Galerie wartet auf Synchronisation:', retryError.message || retryError);
            }
            continue;
          }
          await del(local.id);
  ...
  async function init() {
    ...
    // Lokale Galerie und Bedienung dürfen niemals an Supabase scheitern.
    ...
    if (!window.ParisSync?.gallery) return;
    ...
    try {
      ...
      const pendingLocal = state.photos.filter(photo => photo.syncPending && !remoteIds.has(photo.id));

      // Cloud-Daten mit lokalen, noch ausstehenden Kameraaufnahmen zusammenführen.
      await clearAll();
      for (const remotePhoto of remotePhotos) {
        remotePhoto.syncPending = false;
        await put(remotePhoto);
      }
      for (const localPhoto of pendingLocal) await put(localPhoto);
      state.urls.forEach(url => URL.revokeObjectURL(url));
      state.urls.clear();
      state.photos = [...remotePhotos, ...pendingLocal];
      render();

      for (const localPhoto of pendingLocal) {
        try {
          await window.ParisSync.gallery.upload(localPhoto);
          localPhoto.syncPending = false;
          await put(localPhoto);
        } catch (retryError) {
          console.warn('Galerie-Nachsynchronisierung:', retryError.message || retryError);
        }
      }
      ...
      state.unsubscribe = await window.ParisSync.gallery.subscribe(async payload => {
        try {
          ...
          console.warn('Galerie-Realtime:', error.message || error);
          await reconcileFromCloud();
        }
      }, status => {
      ...
      // Wichtig: Ein Cloud-/Berechtigungsfehler sperrt nicht mehr die Galerie.
      console.warn('Galerie-Cloud derzeit nicht verfügbar:', error.message || error);
      toast('Galerie ist bereit · Cloud-Synchronisierung derzeit nicht verfügbar');
    }
  }
