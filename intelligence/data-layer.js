(function(){
  ...
  const memoryCache=new Map(); let memoryQueue=[]; let memoryErrors=[];
  ...
  function cacheGet(table,scope='all'){return clone(memoryCache.get(cacheKey(table,scope))||null);}
  function cacheSet(table,scope,data){memoryCache.set(cacheKey(table,scope),{data:clone(data),updatedAt:now()});emit('cache:write',{table,scope,count:Array.isArray(data)?data.length:1,volatile:true});return data;}
  function queue(){return clone(memoryQueue);}
  function saveQueue(items){memoryQueue=clone(items||[]);emit('queue:changed',{count:memoryQueue.length,volatile:true});}
  function errors(){return clone(memoryErrors);}
  ...
  function logError(error,operation){const item={id:uuid(),at:now(),message:friendlyMessage(error),code:error?.code||'',operation};const list=[item,...errors()].slice(0,30);memoryErrors=list;state.lastError=item;emit('error',item);}
  ...
  async function executeWrite(type,table,payload,options={}){
    ...
    const c=client();if(!navigator.onLine||!c)throw new Error('Cloud-Verbindung erforderlich. Fachliche Änderungen werden nicht lokal gespeichert.');
    try{
      let q;
    ...
    }catch(error){logError(error,{type,table,payload,options});if(options.queueOnError!==false&&(/fetch|network|offline/i.test(error.message||'')||!navigator.onLine))throw new Error('Cloud-Schreibvorgang fehlgeschlagen; es wurde nichts lokal gespeichert.');throw error;}
  }
  ...
  function invalidate(table){[...memoryCache.keys()].filter(k=>k.startsWith(`${CACHE_PREFIX}${table}:`)).forEach(k=>memoryCache.delete(k));emit('cache:invalidate',{table});}
  ...
  function snapshot(){return {version:VERSION,online:navigator.onLine,queue:queue(),queueCount:queue().length,errors:errors(),lastOperation:state.lastOperation,lastError:state.lastError,cacheKeys:[...memoryCache.keys()],cloudAuthoritative:true,localPersistence:false};}
