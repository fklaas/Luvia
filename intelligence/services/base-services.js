(function(){
  ...
  function pwaScriptUrl(){
    ...
    return new URL('../pwa-service.js?v=11.2.11',base).href;
  }
  ...
  R.register({name:'auth',version:'legacy-adapter-1',description:'Adapter für Supabase-Authentifizierung und Sitzungsstatus.',dependencies:['environment','storage'],async init(){if(!window.ParisAuth)throw new Error('ParisAuth API fehlt.');const client=window.LuviaDatabaseFoundation?.client?.();if(!client)throw new Error('Supabase-Client fehlt.');await window.ParisAuth.ensureInitialSession(client);},status(){const s=window.ParisAuth.getState();return ready({authenticated:s.authenticated,anonymous:s.anonymous,loading:s.loading});},diagnostics(){const s=window.ParisAuth.getState();return{authenticated:s.authenticated,anonymous:s.anonymous,loading:s.loading,email:s.email||'',provider:s.provider||'',userId:s.user?.id||null,lastEvent:s.lastEvent};},test(){const s=window.ParisAuth.getState();return{ok:!s.loading&&Boolean(s.user),message:s.authenticated?'Angemeldete Sitzung erkannt.':s.anonymous?'Anonyme Sitzung erkannt.':'Keine aktive Sitzung.',checks:{initialized:!s.loading,userPresent:Boolean(s.user),sessionPresent:Boolean(s.session)}};}});

  R.register({name:'user',version:'1.0.0-auth-derived',description:'Stellt ein normalisiertes Benutzerprofil für Core-Services bereit.',dependencies:['auth'],status(){const s=window.ParisAuth.getState();return ready({userId:s.user?.id||null});},diagnostics(){const s=window.ParisAuth.getState();return{userId:s.user?.id||null,email:s.email||'',displayName:s.user?.user_metadata?.display_name||s.user?.user_metadata?.first_name||'',provider:s.provider||'',authenticated:s.authenticated,anonymous:s.anonymous};},test(){const s=window.ParisAuth.getState(),id=s.user?.id;return{ok:Boolean(id),message:id?'Benutzerkontext normalisiert.':'Keine User-ID vorhanden.',checks:{userId:Boolean(id),provider:Boolean(s.provider||s.anonymous),metadataReadable:Boolean(s.user?.user_metadata||{})}};}});
